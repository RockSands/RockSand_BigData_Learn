...
public class UpdateBolt implements IRichBolt {
		...
		if (List.class.isInstance(input.getValue(0))) {
			for (Object o : (List) input.getValue(0)) {
				if (OggRecord.class.isInstance(o)) {
					...
					System.out.println("=更新=>{" + oggRecord.getBefore().getName().trim() + " , "
							+ oggRecord.getBefore().getValue() + " -> " + oggRecord.getAfter().getValue());
					offset = offset + (oggRecord.getAfter().getValue() - oggRecord.getBefore().getValue());
				}
			}
		}
		if (offset != 0) {
