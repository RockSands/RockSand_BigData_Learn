...
public class DeleteBolt implements IRichBolt {
		...
		if (List.class.isInstance(input.getValue(0))) {
			for (Object o : (List) input.getValue(0)) {
				if (OggRecord.class.isInstance(o)) {
					...
					System.out.println("=删除=>" + oggRecord.getBefore().getName().trim() + " , "
							+ oggRecord.getBefore().getValue());
					offset = offset - oggRecord.getBefore().getValue();
				}
			}
		}
		if (offset != 0) {
